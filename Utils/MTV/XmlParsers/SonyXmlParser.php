 ...
 * @author			German Dosko
 * @version			June 6, 2012
 * @filesource		/Utils/MTV/XmlParsers/SonyXmlParser.php
 */
class SonyXmlParser extends AbstractXmlParser {
	...
	public function getFcsField($partnerField){
		...
		switch ($partnerField) {
			...
			default:
				throw new Exception('Unexpected Sony field "'.$partnerField.'". SonyXmlParser->GetFcsFields().');
				break;
		}
	}
	...
	private function setCountryValues(&$bundle, $xml){
		...
		if(!is_object($bundle->getRegion())) {
			throw new Exception('Unable to parse region from xml metadata. SonyXmlParser::GetBundleFromXml().');
		}
