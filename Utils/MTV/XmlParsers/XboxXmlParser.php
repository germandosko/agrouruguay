 ...
 * @author			German Dosko
 * @version			June 6, 2012
 * @filesource		/Utils/MTV/XmlParsers/XboxXmlParser.php
 */
class XboxXmlParser extends AbstractXmlParser {
	...
	public function getFcsField($partnerField){
		...
		switch ($partnerField) {
			...
			default:
				throw new Exception('Unexpected Xbox field "'.$partnerField.'". XboxXmlParser->GetFcsFields().');
				break;
		}
	}
	...
	private function setCountryValues(&$bundle, $xml){
		...
		if(!is_object($bundle->getRegion())) {
			throw new Exception('Unable to parse region from xml metadata. XboxXmlParser::GetBundleFromXml().');
		}
